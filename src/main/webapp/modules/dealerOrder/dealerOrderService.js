function dealerOrderService($log, $q, wydNotifyService, sessionService, $http,
		$location) {
	...
	service.dealersMap = sessionService.dealersMap;

	searchCriteria.isSelectedAll = false;
	searchCriteria.dealer = dealers[0];
	...
	service.onDealerChange = function() {
		if (searchCriteria.dealer.id === 0) {
			searchResult.items = searchResult.orders
		} else {
			var items = _.filter(searchResult.orders, function(item) {
				return item.forUserId === searchCriteria.dealer.id;
			});
	...
	service.proceedToProcessOrder = function() {
		...
		receipt.forUser = searchCriteria.dealer
		receipt.forUserLabel = 'Dealer';
		receipt.forUserUrl = '/dealers/dealer/' + receipt.forUser.id;

		receipt.trans = [];

		addTransaction(orders);

		$location.path('/processDealerOrders');
	};
...
appServices.factory('dealerOrderService', dealerOrderService);