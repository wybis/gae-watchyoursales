function customerTranService($log, $http, $q, wydNotifyService, sessionService,
		$http) {
	...
	service.setForUserById = function(id) {
		...
		var customer = sessionService.customersMap[id];
		if (customer) {
	...
	service.saveReceiptAsTran = function() {
		...
		var path = basePath + '/customerTran'
...
appServices.factory('customerTranService', customerTranService);